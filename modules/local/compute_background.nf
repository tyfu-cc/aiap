...
process COMPUTE_BACKGROUND {
  ...
  tuple(
    val(meta),
    path("${meta.id}_background.result"),
    path("${meta.id}_dichoto_bg.result")
  )
  ...
  rpkm_bin.py background temp.txt \$size ${meta.id}_background.result

  # Calculate background distribution
  bg_total=\$(wc -l < ${meta.id}_background.result)
  bg_half_thres=\$(awk '\$6 <= 0.188 {print \$0}' ${meta.id}_background.result | wc -l)
  bg_less=\$(awk '\$6 <= 0.377 {print \$0}' ${meta.id}_background.result | wc -l)
  bg_more=\$(awk '\$6 > 0.377 {print \$0}' ${meta.id}_background.result | wc -l)
  ...
  echo -e "\$ra_half_thres\\t\$ra_less\\t\$ra_more" > ${meta.id}_dichoto_bg.result
